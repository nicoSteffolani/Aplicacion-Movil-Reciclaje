                  ...
                  child: IconButton(
                    key: Key("button"),
                      icon: Icon(
                        ...
                        color: (locationStatus == LocationStatus.centred || locationStatus == LocationStatus.headingNorth)?Theme.of(context).colorScheme.primary: Theme.of(context).colorScheme.onSurface,
                      ),
                      onPressed: () {
                        centerMap();
                ...
                ClipRRect(
                  ...
                  child: Container(
                    ...
                    child: IconButton(
                      ...
                      onPressed:(mm.centers.isNotEmpty)? () {
...
class __BottomSheetContentState extends State<_BottomSheetContent> {
  ...
  Widget getPlaces(){
    ...
    mm.centers.forEach((center){
      ...
      List<Widget> opened = [Text("${t!.closed}", style: TextStyle(color:Theme.of(context).errorColor)), Text(" ${t.open}: ${center.initTime.format(context)}",style:Theme.of(context).textTheme.caption) ];
      if (DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day, center.stopTime.hour,center.stopTime.minute, 0).isAfter(DateTime.now())){
        opened = [
          Text("${t.opened}"),
                      ...
                      Row(
                        ...
                        children: [
                          OutlinedButton(
                              ...
                              onPressed: () {
                                mm.setPlaceToCenter = Place(id: center.id, lat:center.lat, lng:center.lng,address: center.address, name: center.name,recyclingTypes:center.recyclingTypes);
                    ...
                    Row(
                      ...
                      children: [
                        OutlinedButton(
                            child: Icon(Icons.remove_red_eye),
                            onPressed: () {
                              mm.setPlaceToCenter = Place(id: point.id, lat:point.lat, lng:point.lng,address: point.address, name: point.name,recyclingTypes:point.recyclingTypes);
                              Navigator.pop(context);
                            }
                        ),
                        Container(width: 8),
